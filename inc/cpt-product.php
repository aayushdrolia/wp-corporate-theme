...
{   //wp inbuilt security
    ...
    <p>
        ...
        <input class="widefat" type="text" id="prod_composition" name="prod_composition"
               value="<?php echo esc_attr(get_post_meta($post->ID, 'prod_composition', true)) ?>"/>
    </p>
    <p>
        <label for="prod_packaging">Product Packaging</label>
        <input class="widefat" type="text" id="prod_packaging" name="prod_packaging"
               value="<?php echo esc_attr(get_post_meta($post->ID, 'prod_packaging', true)) ?>"/>
    </p>
    <?php
}
...
function corpo_right_now_content_append()
{
    ...
    ?>
    <style type="text/css">
        li.product-count a:before {
            content: "\f312" !important;
        }

        li.product-category-count a:before {
